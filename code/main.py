...
from generalUtensils import loadCurModel, imageReader, reformatFrame, saveCurModel
from dataPreparation import preProcStart, preProcFromCamera
...
from modelTraining import trainCurModel
from header import *
...
@eel.expose()
def segmentStack(path, nrEdges, savePath):
    print('Segmenting Data Stack.')
    imageStack = imageReader(path, segment=True)
    wearCurve = segmentDataStack(imageStack=imageStack, model=currentModel, nrEdges=int(nrEdges), savePath=savePath)
