...
def writeCSV(resultsVBMax, resultFolder):
    ...
    with open(csvFile, mode='w', newline='') as file: writer = csv.writer(file); writer.writerow(header); writer.writerows(rows)
    return resultFolder, csvFile

def plotWearCurve(filePath, resultFolder):
    ''' Plot the Wear Curve. '''
    df = pd.read_csv(filePath)
    colors = ['blue', 'orange', 'green', 'red']; y_min = 0; y_max = 400
...
def outlierDetection(filePath, resultFolder, cuttingEdges = 4):
    ...
    plt.grid(True, linestyle='--')  # Add grid lines
    plt.tight_layout()
    output_path = join(outlierFolder, 'VB_Curve_DBSCAN.svg') # Full path to save the file
    plt.savefig(output_path, format='svg', dpi=300) # Save the plot as a SVG file
    ...
    for idx in range(cuttingEdges):
        ...
        labels = clustering.labels_
        plt.figure(figsize=(12,8))
        classMemberMask = (labels != -1); xy = data2D[classMemberMask] # Cluster
        plt.scatter(xy[:, 0], xy[:, 1], c=colors[idx], edgecolor='k', label=f'{idx + 1}st Tooth Clusters')
        classMemberMask = (labels == -1); xy = data2D[classMemberMask] # Outlier
...
def testDBSAN():
    resultFolder = join(getcwd(), 'projects', 'milling', 'opcSegmentingProject', 'results', '2024_0811_041641')
    resultFile = join(resultFolder, '2024_0811_042223.csv')
    outlierDetection(resultFolder=resultFolder, filePath=resultFile)
