...
import cv2
import pandas as pd
from os import makedirs, listdir, getcwd, chmod, umask
...
def wearDetectionStack(dataStack, nrEdges, projectPath):
    ...
    resultFolder, resultFile = writeCSV(resultsVBMax=resultsVBMax, projectPath=projectPath)
    return resultsVBMax, resultFolder, resultFile

def writeCSV(resultsVBMax, projectPath):
    ...
    csvFolder = join(projectPath,'results', 'wearCurve'+str(getTimeStamp()))
    csvFile = join(csvFolder,str(getTimeStamp())+'.csv')
    makedirs(csvFolder)
    ...
    return csvFolder, csvFile

def plotWearCurve(filePath, resultFolder):
    print('Plotting Wear Curve')
    ...
    output_path = join(resultFolder, 'VB_Curve.svg')
